...
import com.example.geogram.AdminActivity;
import com.example.geogram.ChatActivity;
import com.example.geogram.Chatroom;
import com.example.geogram.ChatroomActivity;
...
import com.example.geogram.SettingsActivity;
...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        //SITUATION: Application is in background or closed
            ...
            if(identifyDataType.equals(getString(R.string.data_type_admin_broadcast))){
                ...
                //we need to chatroomId because we need to get details for the chatroom messages was post
    ...
    private void sendChatmessageNotification(String title, String message, Chatroom chatroom){
        ...
        builder.setSmallIcon(R.drawable.home)//tabian_consulting_logo
                .setLargeIcon(BitmapFactory.decodeResource(getApplicationContext().getResources(),
                        R.drawable.home))//tabian_consulting_logo
                .setSound(RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION))
                .setContentTitle(title)
                .setContentText("New messages in " + chatroom.getChatroom_name())
                //.setColor(getColor(R.color.blue4))
                .setAutoCancel(true)//when one user push it will cancel from the notification bar
                .setSubText(message)//bottom e j text dekha jai
                .setStyle(new NotificationCompat.BigTextStyle() //divide the big text and the summery text
                        .bigText("New messages in " + chatroom.getChatroom_name()).setSummaryText(message))
                .setNumber(mNumPendingMessages)//to set the number of messages
